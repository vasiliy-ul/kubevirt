...
const (
	...
	secret  = "Cg05q+Ifo83oOrAlJ807Nk0/bYBECba/j3ttqzfG+i5wydGnCTESB8ODlnoXNjv3lu7QvEgzt8o+13QalUmgng=="
	header  = "AAAAAGbt+KNbn0xsybLEqHlJ8+a/tDMFsw4yLKb7X09NSVhmwY8le4QEly5DVKy6zgqxew=="
)
...
func (o *Command) Run(args []string) error {
	...
	case COMMAND_FSLIST:
		...
		/*if sevPlatformInfo, err := virtClient.VirtualMachineInstance(namespace).SEVFetchCertChain(vmiName); err != nil {
			...
			if err != nil {
				return fmt.Errorf("Cannot marshal SEV platform info %v", err)
			}
			fmt.Printf("%s\n", string(data))
		}*/
		// SEVSetupSession
		...
		fmt.Printf("1) %v\n", sevSessionOptions)
		if err := virtClient.VirtualMachineInstance(namespace).SEVSetupSession(vmiName, sevSessionOptions); err != nil {
			return fmt.Errorf("Error %s/%s: %v", namespace, vmiName, err)
		} else {
			fmt.Printf("2) %v\n", sevSessionOptions)
		}*/
		// SEVQueryLaunchMeasurement
		/*if sevMeasurementInfo, err := virtClient.VirtualMachineInstance(namespace).SEVQueryLaunchMeasurement(vmiName); err != nil {
			return fmt.Errorf("Error %s/%s: %v", namespace, vmiName, err)
		} else {
		...
		fmt.Printf("1) %v\n", sevSecretOptions)
		if err := virtClient.VirtualMachineInstance(namespace).SEVInjectLaunchSecret(vmiName, sevSecretOptions); err != nil {
			return fmt.Errorf("Error %s/%s: %v", namespace, vmiName, err)
		} else {
			fmt.Printf("2) %v\n", sevSecretOptions)
		}*/
